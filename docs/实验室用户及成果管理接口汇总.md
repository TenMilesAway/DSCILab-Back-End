...
| 接口 | 方法 & 地址 | 权限 | 请求字段 | 返回 |
| --- | --- | --- | --- | --- |
| 用户自助上传头像 | `POST /lab/users/crud/profile/photo` | 登录 | `multipart/form-data`，字段优先 `photofile`（同时兼容 `avatarfile`、`file`，文件必选） | `UploadFileDTO`（字段见[2.7](#27-用户相关通用数据结构)） |
| 管理员上传指定用户头像 | `POST /lab/users/crud/{userId}/photo` | 仅管理员（`@labUserPermission.isAdmin()`） | 同上，另需 path `userId` | `UploadFileDTO` |

管理员上传指定用户头像补充说明：
- 接口语义：管理员为指定 `lab_user` 记录上传并更新头像。
- 路径参数：`userId` 为目标实验室用户 ID，必须存在，否则返回 `USER_NON_EXIST`。
- 请求方式：`multipart/form-data`，优先使用文件字段 `photofile`；同时兼容 `avatarfile`、`file`。
- 文件处理：复用 `FileUploadUtils.upload(UploadSubDir.AVATAR_PATH, file)`，沿用系统统一的大小限制、扩展名白名单和本地文件存储。
- 存储路径：上传根目录来自 `agileboot.file-base-dir`，必须配置为可写绝对路径；开发环境默认使用 `${user.home}/agileboot/files`，代码会自动拼接为 `${user.home}/agileboot/files/profile/avatar/`。
- 文件格式：头像实际应使用图片格式，沿用系统白名单中的 `bmp`、`gif`、`jpg`、`jpeg`、`png`。
- 数据落点：仅更新 `lab_user.photo`，并同步更新 `updater_id`、`update_time`，不会改动其他用户资料字段。
- 成功响应：`ResponseDTO<UploadFileDTO>`，其中 `data.imgUrl` 为头像访问相对路径。
- 审计：接口已按现有方式记录 `AccessLog`，业务类型为修改。
