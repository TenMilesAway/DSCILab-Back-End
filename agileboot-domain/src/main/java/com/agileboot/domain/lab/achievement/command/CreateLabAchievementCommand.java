 ...
 */
@Data
@Schema(description = "创建成果命令")
public class CreateLabAchievementCommand {
    ...
    @Schema(description = "成果类型：1=论文,2=项目", required = true)
    @NotNull(message = "成果类型不能为空")
    @Min(value = 1, message = "成果类型值必须在1-2之间")
    @Max(value = 2, message = "成果类型值必须在1-2之间")
    private Integer type;

    @Schema(description = "论文类型：1-7（仅当type=1时有效）")
    @Min(value = 1, message = "论文类型值必须在1-7之间")
    @Max(value = 7, message = "论文类型值必须在1-7之间")
    private Integer paperType;

    @Schema(description = "项目类型：1-8（仅当type=2时有效）")
    @Min(value = 1, message = "项目类型值必须在1-8之间")
