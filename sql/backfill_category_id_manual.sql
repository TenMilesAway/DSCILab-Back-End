...
-- Steps: (1) START TRANSACTION; (2) Execute this script; (3) Verify counts; (4) COMMIT; if anything looks wrong, ROLLBACK.
...
-- 7) How to rollback if needed (run inside the same connection/tx or later):
-- UPDATE lab_achievement a JOIN <the backup table name> b ON a.id=b.id SET a.category_id=b.old_category_id;
-- Example (replace with the actual backup name printed above):
-- UPDATE lab_achievement a JOIN lab_achievement_category_bk_20250918_120000 b ON a.id=b.id SET a.category_id=b.old_category_id;
